...
pub const SignedLeb128 = struct {
    ...
    pub fn encode(self: SignedLeb128, writer: anytype) !usize {
...
pub const UnsignedLeb128 = struct {
    ...
    pub fn encode(self: UnsignedLeb128, writer: anytype) !usize {
        var bytes_written: usize = 0;
        var value = self.value;
...
fn lowBits(value: u64) u8 {
    ...
    return @intCast(lower_eight_bits & ~CONTINUATION_BIT);
}
...
test "encodes signed leb 128" {
    ...
    for (to_encode, expected_encoding) |value_to_encode, expected| {
        ...
        _ = try encoder.encode(buf.writer());
