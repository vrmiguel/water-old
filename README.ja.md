...
`water` は、WebAssembly の人間が読み書きできるテキスト形式を扱う、最小限ながら効率的なパーサー兼コンパイラです。WAT モジュール、命令、インポート、関数、その他の WebAssembly コンポーネントを解析するための基盤を提供します。
...
- `src/leb128.rs` - LEB128 可変長整数エンコード
...
これは、WebAssembly テキスト形式の解析とコンパイルを理解することに焦点を当てた、実験的/教育的なプロジェクトです。
