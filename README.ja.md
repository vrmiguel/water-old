...
Rust で書かれた、軽量で高性能な WebAssembly Text Format (WAT) 向けコンパイラです。

## 概要

`water` は、WebAssembly の人間が読みやすいテキスト形式に対応した、最小限ながら効率的なパーサー兼コンパイラです。WAT モジュール、命令、インポート、関数、その他の WebAssembly コンポーネントを解析するための基盤を提供します。

## 機能

- **高速な解析** - `nom` を使った効率的な WAT 命令およびモジュール解析
...
これは、WebAssembly Text Format の解析とコンパイルを理解することに重点を置いた、実験的かつ教育的なプロジェクトです。
